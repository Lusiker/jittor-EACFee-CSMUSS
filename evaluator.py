...
def get_dataset(args, mode, threshold=None, match=None):
    gt_path = os.path.join(args.data_path, f'{mode}-segmentation') # ground truth
    # predict_path = os.path.join(args.predict_path, mode)
    dataset = EvalDataset(args.predict_path, gt_path, threshold=threshold, match=match) # args.predict_path原为predict_path
    dataset.set_attrs(
        batch_size=1,
        shuffle=False,
