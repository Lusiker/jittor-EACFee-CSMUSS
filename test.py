...
def main_worker(args):
    ...
    if args.mode != 'match_generate':
        ...
        # NOTE：由于提交验证时需要生成的结果在根目录下，因此将此处改为绝对地址
        with open("weights/final_result/match_unet.json", 'r') as match1_raw:
            match1 = json.loads(match1_raw.read())
    ...
    if args.mode == 'match_generate':
        ...
        target_path = os.path.join(args.dump_path, identifier)
...
if __name__ == '__main__':
    ...
    # 更换成对应的数据集路径
    args.data_path = "../PASS/data/ImageNetS50"

    # 
    args.arch = "resnet18"
    args.pretrained = "./weights/final_result/checkpoint.pth.tar"
    args.model1_name = "unet"
    args.model_tta_method = "flip|blur"
    args.dump_path = "./"
    args.mode = "test"
    args.num_classes = 50
    args.is_test_b = 1
    args.match_file = "./weights/final_result/match_unet.json"
