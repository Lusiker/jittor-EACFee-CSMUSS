...
class UpSampleBlock(nn.Module):
    ...
    def execute(self, x_new, x_from_left):
        x_new = self.deconv(x_new)
        
        padding_1 = x_from_left.shape[2] - x_new.shape[2]
        padding_2 = x_from_left.shape[3] - x_new.shape[3]
        jt.nn.pad(x=x_new, padding=(0, 0, padding_1, padding_2))

        concated = jt.concat((x_from_left, x_new), dim=1)
...
class UpSample(nn.Module):
    def __init__(self, class_nums):
        ...
        self.out_conv = nn.Conv2d(64, class_nums + 1, 1, 1)

    def execute(self, x4, x3, x2, x1, x0):
        u = self.up1(x4, x3)
        u = self.up2(u, x2)
        u = self.up3(u, x1)
        u = self.up4(u, x0)
...
class ResNet(nn.Module):
    def __init__(self,
        ...
        self.is_unet = apply_unet
        if isinstance(self.shallow, int):
            self.shallow = [self.shallow]

        self.inplanes = width_per_group * widen # 64
        self.dilation = 1
        ...
        num_out_filters = width_per_group * widen # 64
        self.conv1 = nn.Conv2d(3,
                               num_out_filters, # 3 --> 64
                               kernel_size=7,
                               stride=2,
                               padding=2,
                               bias=False)
        ...
        num_out_filters *= 2 # 128
        self.layer2 = self._make_layer(block,
                                       num_out_filters,
                                       layers[1],
                                       stride=2,
                                       dilate=replace_stride_with_dilation[0])
        num_out_filters *= 2 # 256
        self.layer3 = self._make_layer(block,
                                       num_out_filters,
                                       layers[2],
                                       stride=2,
                                       dilate=replace_stride_with_dilation[1])
        num_out_filters *= 2 # 512
        ...
        if train_mode == 'pixelattn':
            ...
            if self.is_unet:
                self.last_layer = UpSample(num_classes)
            else:
                self.last_layer = nn.Conv2d(num_out_filters * block.expansion, num_classes + 1, 1, 1)
    ...
    def execute_backbone(self, x, avgpool=True):
        x = self.padding(x) # H * W * 3 --> H+2 * W+2 * 3 w/ padding 0.0

        x = self.conv1(x) # H+2 * W+2 * 3 --7*7kernel--> 64 channels
        x = self.bn1(x) # batch norm layer
        x = self.relu(x)
        x0 = x
        x = self.maxpool(x)
    ...
    def execute(self, inputs, gridq=None, gridk=None, mode='train'):
        ...
        if self.train_mode == 'finetune':
            if self.is_unet:
                out4, out3, out2, out1, out0 = self.execute_backbone(inputs)

                return self.last_layer(out4, out3, out2, out1, out0)
            else:
                return self.last_layer(self.execute_backbone(inputs))
